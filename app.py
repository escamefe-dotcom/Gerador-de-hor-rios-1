...
# --------------------------------------------------------------------------------------
# --- BLOCO DE ANÁLISE ESTÁTICA (T1-T5) - CORRIGIDO COM OS NOVOS DADOS ---
# --------------------------------------------------------------------------------------
...
def obter_analise_finais_estatica() -> Tuple[pd.DataFrame, Dict[int, int], int]:
    ...
    incluindo um Dicionário de Ranking dos Finais de Minuto.
    """
    
    # CORREÇÃO: NOVAS FREQUÊNCIAS DOS FINAIS DE MINUTO
    # T1(2), T2(6), T3(9), T4(0), T5(5)
    ...
    # CORREÇÃO: NOVO TOP 1 MINUTO ISOLADO (12)
    top_1_minuto_completo = 12 
    
    # Criando o DataFrame para exibição na sidebar
    dados_exibicao = [
        {'Sinal': "🎯", 'Minuto Exemplo': f"{top_1_minuto_completo:02d}", 'Final': f"{2} (Minuto Isolado)"},
        {'Sinal': "🔥", 'Minuto Exemplo': "02", 'Final': 'T1(2)'},
        {'Sinal': "🔥", 'Minuto Exemplo': "06", 'Final': 'T2(6)'},
        {'Sinal': "🔥", 'Minuto Exemplo': "09", 'Final': 'T3(9)'}, 
        {'Sinal': "🔥", 'Minuto Exemplo': "00", 'Final': 'T4(0)'},
        {'Sinal': "🔥", 'Minuto Exemplo': "05", 'Final': 'T5(5)'},
    ]
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    def adicionar_horario_faltante(soma: int, origem_completa: str):
        ...
        horarios_brutos_list.append({
            ...
            'Origem_Bruta': origem_simples, # V ou VT
            'Rodada': str(rodada),
            'R': r_formatado,
            'C': cotacao_C,
            'Sub_Origem_C': '' 
    ...
    for alerta_c in alertas_confluencia:
        ...
        # *** REGRA DE FILTRO: Só entra se houver V ou VT no mesmo minuto ***
        if horario_alvo_c_minuto in horarios_faltantes_alvo_set:
            
            horarios_brutos_list.append({
                'Timestamp_dt': alerta_c['Timestamp_dt'],
                'Origem_Bruta': 'C', # AQUI ADICIONA O C
                'Rodada': str(rodada),
                'R': r_formatado, 
                'C': cotacao_C,
                'Sub_Origem_C': alerta_c['Origem_C'] # Detalhe do C
            })
        
    horarios_brutos_list.sort(key=lambda x: x['Timestamp_dt'])
    
    # 3. CONSOLIDAÇÃO INTERNA DE ALERTAS (Agrupamento V, VT, C no mesmo minuto/segundo)
    ...
    while i < len(horarios_brutos_list):
        ...
        # Consolida Origens Brutas (V, VT, C) em uma string única
        origens_consolidadas = sorted(list(set(item['Origem_Bruta'] for item in grupo)))
        sub_origens_confluencia = [item['Sub_Origem_C'] for item in grupo if item['Sub_Origem_C']]
        ...
        horarios_consolidados.append({
            ...
            'Origem': ' / '.join(origens_consolidadas), # GERA STRING 'C / V / VT'
            'R': r_consolidado,
            'C': c_consolidado, 
            'Timestamp_dt': horario_final_dt,
            'Sub_Origem_C': ' / '.join(sorted(list(set(sub_origens_confluencia)))) 
        })
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    for _, row in historico_completo.iterrows():
        ...
        if chave_consolidada not in consolidado_dict:
            # Novo Alerta: Armazena as origens como um SET de strings
            consolidado_dict[chave_consolidada] = {
                'Rodadas': {row['Rodada']},
                'Origens': set(row['Origem'].split(' / ')), # Armazena {'V', 'VT', 'C'}
                'R': row['R'],
                'C': row['C'],
                'Sinalizacao': sinalizacao, 
                'Sub_Origem_C': {row.get('Sub_Origem_C', '')}, 
                'Timestamp_dt': row['Timestamp_dt']
            }
        else:
            # Alerta Existente: Atualiza Rodadas, Origens e Sub_Origens
            consolidado_dict[chave_consolidada]['Rodadas'].add(row['Rodada'])
            
            # Fusão robusta das Origens
    ...
    for horario, data in consolidado_dict.items():
        ...
        # Converte o SET de Origens de volta para STRING ordenada (C / V / VT)
            ...
            with st.spinner(f'Analisando Rodada {rodada} e consolidando histórico...'): 
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    with st.sidebar:
                        ...
                        st.subheader("📊 Top Minutos (Estático) - Novos Dados")
                        st.dataframe(
                        ...
                        st.info(f"Classificação dos Finais: T1(2), T2(6), T3(9), T4(0), T5(5).")
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Origem": st.column_config.TextColumn(
                "Origem (V, VT, C)",
                help="V=Faltantes Vela; VT=Soma Faltantes; C=Confluência Numérica. O sinal C só aparece se coincidir com V ou VT."
            ),
            "Confluência": st.column_config.TextColumn(
                "Det. Confluência",
                help="Detalhes dos padrões de Confluência Numérica (Vela, H:Residuo, Rodada)"
            )
        }
    )
