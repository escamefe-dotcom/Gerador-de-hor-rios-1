...
if 'historico_alertas' not in st.session_state:
    ...
    st.session_state.historico_alertas = st.session_state.historico_alertas.astype({
        ...
        'Sinalizacao': 'object', 'Sub_Origem_C': 'object', 'Origem': 'object'
    })
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    for col in historico_atualizado.columns:
        if col not in novo_df.columns: 
            novo_df[col] = '-' if col in ['R', 'C', 'RA_Soma', 'Resultado_Vela'] else None 
    
    # Filtra alertas antigos (mantém apenas os alertas criados a partir do horário base da rodada inserida)
    ...
    # Renomeia temporariamente 'Rodada_Resultante' para evitar conflito de tipo na concatenação se um for None e outro str
    novo_df_limpo = novo_df.copy()
    
    # Preenche NaNs com strings compatíveis para evitar erros de concatenação de tipos
    for col in ['Rodada_Resultante', 'RA_Soma', 'R', 'C', 'Resultado_Vela']:
         if col in novo_df_limpo.columns:
        ...
        try:
            ...
            with st.spinner(f'Analisando Rodada {rodada_int} e consolidando histórico...'): 
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    # Adiciona a coluna Resultado_Vela com valor default
                    if 'Resultado_Vela' not in novo_df_bruto.columns:
                        novo_df_bruto['Resultado_Vela'] = '-'
