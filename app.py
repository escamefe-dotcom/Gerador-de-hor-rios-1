...
if not st.session_state.historico_alertas.empty:
    ...
    # Prepara as colunas para exibição
    df_exibicao['Horário'] = df_exibicao['Sinalização'] + ' ' + df_exibicao['Horário Focado']
    df_exibicao['Rodada(s)'] = df_exibicao['Rodada'] # Coluna da Rodada (ajuste de exibição)
    df_exibicao['Confluência'] = df_exibicao['Sub_Origem_C'].apply(lambda x: x if x else '-')
    
    # Exclui as colunas internas/redundantes antes de mostrar
    ...
    st.dataframe(
        df_exibicao, 
        hide_index=True,
        column_order=['Horário', 'Rodada(s)', 'R', 'C', 'Origem', 'Confluência'], # Ordem ajustada para incluir Rodada(s)
        column_config={
            "Horário": st.column_config.TextColumn(
                "Horário Focado (Sinalizado)", 
                help="Sinalização de prioridade (T1 a T5)." 
            ),
             "Rodada(s)": st.column_config.TextColumn(
                "Rodada(s)",
                help="Número(s) da Rodada que gerou(aram) o alerta."
            ),
