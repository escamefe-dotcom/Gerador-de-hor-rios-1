...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    try:
        ...
        # Garante que a vela seja lida com ponto para float
    ...
    # --- NOVO: COLETA DOS HORÁRIOS ALVO DE FALTANTES (V e VT) PARA FILTRO ---
    ...
    def adicionar_horario_faltante(soma: int, origem_completa: str):
        ...
        # 1. Adiciona o Alerta Bruto
        horarios_brutos_list.append({
            'Timestamp_dt': novo_horario_dt, 
            'Origem_Bruta': origem_simples, # V, VT
            'Rodada': str(rodada),
            'R': r_formatado,
            'C': cotacao_C,
            'Origem_C': '' 
        ...
        # 2. Adiciona o Horário Alvo (Apenas Hora:Minuto) para o Filtro de C
        if origem_simples in ['V', 'VT']:
    ...
    for alerta_c in alertas_confluencia:
        ...
        if horario_alvo_c_minuto in horarios_faltantes_alvo_set:
            ...
            horarios_brutos_list.append({
                ...
                'R': r_formatado, 
                'C': cotacao_C,
                'Origem_C': alerta_c['Origem_C'] # Detalhe do C
            })
    ...
    while i < len(horarios_brutos_list):
        ...
        # Determina o Horário Focado do grupo
        if len(grupo) % 2 == 1:
        ...
        # Consolida Origens Brutas (V, VT, C)
        ...
        horarios_consolidados.append({
            ...
            'Origem': ' / '.join(origens_consolidadas), # Corrigido para incluir V, VT e C
            'R': r_consolidado,
            'C': c_consolidado, 
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    for _, row in historico_completo.iterrows():
        ...
        # 1. Prioridade Máxima: TOP 1 MINUTO ISOLADO
        ...
        if chave_consolidada not in consolidado_dict:
            consolidado_dict[chave_consolidada] = {
                'Rodadas': {row['Rodada']},
                'Origens': {row['Origem']}, # Usa a Origem já consolidada (V / VT / C)
                'R': row['R'],
                'C': row['C'],
            ...
            # Se a linha de Origem já é consolidada (ex: V / VT), a adiciona ao set
            origens_atuais = set(consolidado_dict[chave_consolidada]['Origens'].pop().split(' / '))
            origens_atuais.update(set(row['Origem'].split(' / ')))
            consolidado_dict[chave_consolidada]['Origens'] = {' / '.join(sorted(list(origens_atuais)))}
    ...
    for horario, data in consolidado_dict.items():
        ...
        dados_finais.append({
            ...
            'Origem': list(data['Origens'])[0], # Usa a string de Origem consolidada
            'R': data['R'],
            'C': data['C'],
