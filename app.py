...
def calcular_faltantes_seletivos(digitos_presentes: List[int]) -> Tuple[List[int], int]:
    ...
    A regra é: encontrar a sequência contígua de 4 dígitos (Ex: d, d+1, d+2, d+3) 
    ...
    faltantes_globais = sorted(list(todos_digitos - set(digitos_presentes)))
    
    melhor_sequencia_faltantes = []
    ...
    for inicio_seq in range(10):
        ...
        # A sequência contígua é sempre [d, d+1, d+2, d+3]
        ...
        # 3. Condição principal: a sequência deve ter EXATAMENTE 2 faltantes.
        if len(digitos_presentes_na_seq) == 2 and len(digitos_faltantes_na_seq) == 2:
            
            # 4. Prioriza a sequência que começa com o menor dígito
    ...
    if melhor_sequencia_faltantes:
        ...
        # Fallback: Se não for possível formar uma sequência de 4 com 2 faltantes, 
        # pegamos os 2 menores faltantes globais como fallback seguro.
        faltantes_finais = faltantes_globais[:2] 
