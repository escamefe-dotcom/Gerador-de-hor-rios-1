...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    def adicionar_horario(soma: int, origem_completa: str):
        ...
        # --- NOVO: Lógica para extrair o nome simplificado (V, H, VT, HT, HM) ---
        if origem_completa.startswith('V x'):
            origem_simples = 'V'
        elif origem_completa.startswith('H x'):
            origem_simples = 'H'
        else:
            # Para VT, HT, HM, o nome é a primeira palavra antes do espaço
    ...
    while i < len(horarios_brutos_list):
        ...
        # Regra de Consolidação: Pega o último ou o do meio
        if len(grupo) % 2 == 1:
        ...
        # Consolida origens (usando os nomes simplificados: V, H, VT, etc.)
        ...
        horarios_consolidados.append({
            ...
            'Origem': ' / '.join(origens_consolidadas), # Ex: 'V / HT'
            'R': r_formatado,
...
if not st.session_state.historico_alertas.empty:
    ...
    # --- NOVO: Remove a coluna 'Rodada' da exibição ---
