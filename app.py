...
st.set_page_config(
    ...
    layout="centered",
    initial_sidebar_state="auto", # Mudança para 'auto' ou 'expanded' para mostrar a sidebar
)

# --- FUNÇÕES DE CÁLCULO GERAL ---
# (Funções: calcular_cotacao, calcular_faltantes_seletivos, formatar_resultado_r)
# Estas funções permanecem inalteradas, mas são omitidas aqui por brevidade
...
# 3.2. Botão de Ação (A ser processado abaixo)
            ...
            with st.spinner(f'Analisando Rodada {rodada} e consolidando histórico...'): 
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    with st.sidebar:
                        ...
                        st.info(f"Finais Minutários p/ Alertas: {', '.join(sinalizacao_finais)}")
...
if not st.session_state.historico_alertas.empty:
    ...
    # Adiciona a coluna de sinalização no início do horário para o display
