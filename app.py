...
if not st.session_state.historico_alertas.empty:
    ...
    # 2. CORREÇÃO DE ERRO: Cria a coluna que mostra RA_Soma (se houver) ou a Rodada (se for V/VT/C)
    df_exibicao['RA Soma/Rodada ID'] = df_exibicao.apply(
    ...
    st.dataframe(
        df_exibicao, 
        hide_index=True,
        column_order=['Horário', 'RA Soma/Rodada ID', 'R', 'C', 'Origem', 'Det. RA/C'], 
        column_config={
            "Horário": st.column_config.TextColumn(
                "Horário Focado (Sinalizado)", 
                help="Sinalização de prioridade (T1 a T5)." 
            ),
             "RA Soma/Rodada ID": st.column_config.TextColumn(
                "RA Soma / Rodada ID",
