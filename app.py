...
# Apenas para display na sidebar - não mais para sinalização.
@st.cache_data(show_spinner=False)
...
@st.cache_data(show_spinner=False)
def analisar_top_minutos_completos(df_bruto: pd.DataFrame) -> Tuple[pd.DataFrame, List[int], int]:
    """
    Filtra o DataFrame bruto para encontrar a frequência dos minutos completos mais pagantes (Vela > 10.00x).
    Sinaliza o Top 1 com 🚀 e o restante do Top 5 com 🔥 na coluna Minuto para display.
    Retorna o DF para display e a lista de minutos (int) para sinalização.
    """
    if df_bruto.empty:
        return pd.DataFrame(columns=['Minuto', 'Contagem']), [], None
    ...
    # Lista de minutos (int) para o novo sistema de sinalização (00-59)
    top_minutos_int = contagem_minutos.head(5)['Minuto'].tolist()
    top_1_minuto_completo = top_minutos_int[0] if top_minutos_int else None
    
    # Formata para display no dataframe
    ...
    return df_top_5[['Minuto', 'Contagem']], top_minutos_int, top_1_minuto_completo
...
def consolidar_historico(novo_df, horario_base_dt, top_minutos_00_59_list: List[int], top_1_minuto_00_59: int):
    ...
    nos minutos completos (00-59) mais pagantes.
    """
    ...
    for _, row in historico_completo.iterrows():
        ...
        # 1. Determina a sinalização (FOGUETE 🚀 ou FOGO 🔥) - AGORA USANDO MINUTOS 00-59
    ...
    if len(linhas) < 3:
        ...
        try:
            ...
            with st.spinner(f'Analisando Rodada {rodada} e consolidando histórico...'): 
                ...
                # 2. Analisa os TOP 5 Finais Minutários (Para display na sidebar) e Minutos Completos (Para sinalização)
                top_minutos_finais, top_1_final = analisar_top_minutos_finais_pagantes(st.session_state.historico_bruto) 
                df_top_minutos_completos, top_minutos_completos_int, top_1_minuto_completo = analisar_top_minutos_completos(st.session_state.historico_bruto)
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    # 4. Consolida Alertas e aplica Sinalização (AGORA USANDO MINUTOS COMPLETOS)
                    ...
                    with st.sidebar:
                        ...
                        # Mantendo esta info, mas deixando claro que a sinalização principal é 00-59
                        st.info(f"Top Finais Minutários (0-9): {', '.join(sinalizacao_finais)}")
    ...
    st.dataframe(
        ...
        column_config={
            "Horário": st.column_config.TextColumn(
                ...
                help="🚀 Top 1 Minuto Completo (00-59) / 🔥 Top 5 Minuto Completo (00-59)" # Ajuda atualizada
            )
        }
    )
