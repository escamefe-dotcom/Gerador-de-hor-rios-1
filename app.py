...
# --------------------------------------------------------------------------------------
# --- BLOCO DE ANÁLISE ESTÁTICA (BASEADO NA FREQUÊNCIA DO TOP 5) ---
# --------------------------------------------------------------------------------------
...
def obter_analise_finais_estatica() -> Tuple[pd.DataFrame, List[int], int]:
    ...
    # Criando o DataFrame para exibição (ajustado para melhor compatibilidade com emojis no st.dataframe)
    dados_exibicao = [
        {'Sinal': "🚀", 'Minuto Exemplo': f"{top_1_minuto_completo:02d}", 'Final': f"{top_1_final_minuto} (Top 1)"},
        {'Sinal': "🔥", 'Minuto Exemplo': "08", 'Final': '8'},
        {'Sinal': "🔥", 'Minuto Exemplo': "00", 'Final': '0'},
        {'Sinal': "🔥", 'Minuto Exemplo': "02", 'Final': '2'},
        {'Sinal': "🔥", 'Minuto Exemplo': "03", 'Final': '3'},
    ]
            ...
            with st.spinner(f'Analisando Rodada {rodada} e consolidando histórico...'): 
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    with st.sidebar:
                        ...
                        # CORREÇÃO AQUI: Usando o nome da coluna correto ('Tipo de Minuto') para exibição
                        st.dataframe(
                            df_top_minutos_completos,
                            hide_index=True,
                            column_config={
                                "Sinal": st.column_config.TextColumn(""), # Coluna para o símbolo (🚀/🔥)
                        ...
                        st.info(f"A sinalização agora usa os Finais: 7 (🚀), 8, 0, 2, 3 (🔥)")
...
if not st.session_state.historico_alertas.empty:
    ...
    # Esta linha deve garantir que o símbolo seja inserido na coluna 'Horário'
    ...
    st.dataframe(
        ...
        column_config={
            "Horário": st.column_config.TextColumn(
                ...
                help="🚀 Top 1 Minuto Completo (00-59) / 🔥 Top 5 Finais (7, 8, 0, 2, 3)"
            )
        }
    )
