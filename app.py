...
def calcular_faltantes_seletivos(digitos_presentes: List[int]) -> Tuple[List[int], int]:
    ...
    1. O faltante do menor buraco.
    2. O vizinho de fechamento do maior buraco.
    """
    ...
    # Adiciona o primeiro dígito + 10 ao final para fechar o ciclo
    digitos_ciclicos = digitos_presentes + [digitos_presentes[0] + 10]
    
    buracos = []
    
    # 1. Identificar e listar todos os buracos (d1 -> d2)
    for i in range(len(digitos_presentes)):
        ...
        if diferenca > 1:
            ...
            # d1 é o dígito presente antes do buraco, d2 é o dígito presente depois.
            buracos.append({
                'tamanho': diferenca - 1,
                'faltantes': faltantes,
                'd1_presente': d1,
                'd2_presente': d2_real % 10
            })

    # Se não houver buracos, retorna vazio.
    if not buracos:
        return [], 0

    # 2. Ordenar os buracos por tamanho para identificar o menor e o maior
    buracos_ordenados_tam = sorted(buracos, key=lambda x: x['tamanho'])
    
    buraco_menor = buracos_ordenados_tam[0]
    buraco_maior = buracos_ordenados_tam[-1]
    
    faltantes_finais = []
    
    # --- Lógica de Seleção ---
    
    # A. Primeiro Faltante (Vizinho do Menor Buraco)
    
    # No buraco menor (Ex: 6->8, faltante 7), pegamos o menor faltante.
    if buraco_menor['faltantes']:
        faltantes_finais.append(buraco_menor['faltantes'][0])
    
    # B. Segundo Faltante (Vizinho de Fechamento do Maior Buraco)
    
    if len(faltantes_finais) < 2 and buraco_maior['faltantes']:
        
        # O faltante que "fecha" o maior buraco é o vizinho do dígito presente POSTERIOR.
        # Ex: Buraco 1 -> 6. O vizinho de 6 é 5.
        
        d2_presente = buraco_maior['d2_presente']
        segundo_faltante = (d2_presente - 1) % 10
        
        # Correção para o ciclo (Ex: d2=4, faltante=3. d2=1, faltante=0)
        if d2_presente == buraco_maior['d1_presente'] + 10 % 10:
             # Se for o buraco que fecha o ciclo (Ex: 9->4), o faltante de fechamento é d2_presente + 1
             segundo_faltante = (buraco_maior['d1_presente'] + buraco_maior['tamanho']) % 10

        # Verifica se o segundo faltante já foi selecionado (o que não deve acontecer se os buracos forem diferentes)
        if segundo_faltante not in faltantes_finais:
            # Garante que o número está de fato no buraco
            if segundo_faltante in buraco_maior['faltantes']:
                 faltantes_finais.append(segundo_faltante)


    # 3. Ajuste final (Garante exatamente 2, sem duplicatas)
    
    # Se a lógica acima não encontrou 2 (ex: faltantes iguais ou casos com 3 dígitos presentes e 1 buraco grande)
    if len(faltantes_finais) < 2:
        # Preenche com o próximo faltante não utilizado do maior buraco
        for f in buraco_maior['faltantes']:
             if f not in faltantes_finais:
                 faltantes_finais.append(f)
                 if len(faltantes_finais) == 2:
                     break
    
    # Garante a ordem e que são apenas 2
...
def formatar_resultado_r(r_bruto: float) -> str:
    # ... (Restante da função formatar_resultado_r) ...
    if r_bruto <= 99.99:
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    # ... (Restante da função analisar_e_gerar_alertas) ...
    try:
...
def consolidar_historico(novo_df, horario_base_dt):
    # ... (Restante da função consolidar_historico) ...
