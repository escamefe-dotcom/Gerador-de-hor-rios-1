...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    # Adiciona a coluna Resultado_Vela se estiver faltando
    ...
    # CORREÇÃO: Usar o nome correto 'consolidado_dict'
    Consolidado_dict = {}
...
# --------------------------------------------------------------------------------------
# --- 5. INTERFACE DE ATUALIZAÇÃO MANUAL DE RESULTADO (AGORA ACEITA DADOS COMPLETOS) ---
# --------------------------------------------------------------------------------------

st.markdown("---")
st.subheader("✍️ Adicionar Resultado da Vela")

# Novo campo que aceita os 3 dados para simplificar a cópia
dados_resultado = st.text_area(
    "Cole Horário Focado, Rodada ID (opcional) e Resultado da Vela (em linhas separadas)",
    height=120,
    placeholder="Exemplo:\n11:10:23\n3293215\n15.55x"
)
...
if st.button("Atualizar Resultado", type="secondary"):
    ...
    # O Rodada ID é opcional, mas se existir, pode ajudar a conferir.
    # Vamos focar no Horário e Resultado.
    resultado_vela_input = linhas_res[-1] if len(linhas_res) >= 2 else None
    
    if horario_alvo_res and resultado_vela_input:
        
        # 1. Valida o formato básico do horário
        if not re.match(r'\d{2}:\d{2}:\d{2}', horario_alvo_res):
            st.error("O primeiro campo deve ser o Horário Focado no formato HH:MM:SS.")
        else:
            # 2. Formata o resultado
            resultado_limpo = resultado_vela_input.strip().replace('x', '').replace(',', '.') + 'x'
            
            # 3. Checa se o horário existe no histórico antes de atualizar
...
if not st.session_state.historico_alertas.empty:
    ...
    # 3. Adiciona a coluna Rodada/RA Soma ID de volta (para referência)
    df_exibicao['Rodada/RA ID'] = df_exibicao.apply(
        lambda row: row['RA_Soma'] if row['RA_Soma'] != '-' else row['Rodada'], axis=1
    )
    
    # 4. Cria a coluna de Detalhes
    df_exibicao['Det. RA/C'] = df_exibicao['Sub_Origem_C'].apply(lambda x: x if x else '-')
    
    # 5. Exclui as colunas internas/redundantes antes de mostrar
    ...
    st.dataframe(
        df_exibicao, 
        hide_index=True,
        column_order=['Horário', 'Resultado Vela', 'Rodada/RA ID', 'R', 'C', 'Origem', 'Det. RA/C'], 
        column_config={
            "Horário": st.column_config.TextColumn(
             ...
             "Rodada/RA ID": st.column_config.TextColumn(
                "Rodada/RA ID",
                help="RA Soma (se for RA) ou ID da Rodada que gerou o sinal."
            ),
