...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    try:
        ...
        if not parte_numerica:
             # Retorna '0' como RA_Soma para indicar que não houve alerta RA por soma zero.
        ...
        # st.error(f"Erro de Validação/Parsing: {e}") 
    ...
    if vela_float_total < 10.00:
        ...
        try:
            ...
            for i, offset in enumerate(minuto_offsets):
                if offset > 0:
                    ...
                    horarios_brutos_list.append({
                        ...
                        'Rodada_Resultante': str(rodada_resultante_int), # Rodada Resultante sempre existe para RA
                    })
            
            horarios_consolidados = consolidar_alertas_internamente(horarios_brutos_list)
            # Garante que as colunas sejam strings para concatenação futura
            ...
            return df_final, None, None, '-', '-', horario_base_dt, str(soma_digitos_fracao)
    ...
    # 2. Lógica COMPLETA para VELAS >= 10.00x (R, C, V, VT, Confluência)
        ...
        return df_final, faltantes_V, None, r_formatado, cotacao_C, horario_base_dt, '-'

# --------------------------------------------------------------------------------------
# --- GERENCIAMENTO DE ESTADO E CONSOLIDAÇÃO FINAL DE HISTÓRICO ---
# --------------------------------------------------------------------------------------

# **Ajuste na Inicialização: Garantir tipos de objeto para evitar erros de concatenação**
if 'historico_alertas' not in st.session_state:
    ...
    # Força a tipagem de colunas importantes como 'object' para aceitar strings e None
    st.session_state.historico_alertas = st.session_state.historico_alertas.astype({
        'Rodada': 'object', 'RA_Soma': 'object', 'R': 'object', 'C': 'object', 
        'Resultado_Vela': 'object', 'Rodada_Resultante': 'object'
    })
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    # Garante que todas as colunas existem ANTES de filtrar e concatenar
    for col in ['Resultado_Vela', 'Rodada_Resultante', 'RA_Soma', 'R', 'C']:
        if col not in historico_atualizado.columns: historico_atualizado[col] = '-'
        if col not in novo_df.columns: novo_df[col] = None 
    
    # Filtra alertas antigos
    historico_atualizado = historico_atualizado[
        historico_atualizado['Timestamp_dt'] >= horario_base_dt.replace(second=0, microsecond=0) # Filtra por minuto
    ]
    
    # Garante que os tipos sejam compatíveis antes da concatenação
    novo_df_limpo = novo_df.copy()
    novo_df_limpo['Rodada_Resultante'] = novo_df_limpo['Rodada_Resultante'].fillna('-').astype(str).replace('None', '-')
    novo_df_limpo['RA_Soma'] = novo_df_limpo['RA_Soma'].fillna('-').astype(str)
    
    # Concatenação com o histórico
    ...
    for _, row in historico_completo.iterrows():
        ...
        # Garantir valores de Rodada Resultante e RA_Soma limpos (str ou '-')
        ...
        if chave_consolidada not in Consolidado_dict:
            ...
            # Lógica para priorizar o melhor sinal T
            ...
            # Atualiza Resultado_Vela se estiver vazio
    ...
    for horario, data in Consolidado_dict.items():
        ...
        # ID DE ORIGEM: Rodada Resultante (prioridade) ou Rodada Original
        rodadas_resultantes_validas = [r for r in data['Rodadas_Resultantes'] if r is not None and r != '-']
        ...
        if rodadas_resultantes_validas:
            ...
            # Se não houver Rodada Resultante válida, mostra a Rodada Original
    ...
    if len(linhas) < 3:
        ...
        try:
            ...
            with st.spinner(f'Analisando Rodada {rodada_int} e consolidando histórico...'): 
                ...
                # O horário base é usado para filtrar o histórico antigo
                horario_base_dt_tentativa = datetime.now()
                try:
                    horario_base_dt_tentativa = datetime.strptime(horario_input, "%H:%M:%S")
                except:
                     pass # Mantém o horário atual se o input for inválido
                ...
                # Se a análise falhar em gerar dt, usa o valor da tentativa
                if horario_base_dt is None:
                    horario_base_dt = horario_base_dt_tentativa
                
                if r_final is None or r_final == '-': r_final = 'N/A'
                if cotacao_final is None or cotacao_final == '-': cotacao_final = 'N/A'
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    with st.sidebar:
                        ...
                        if st.button("Limpar Histórico Completo", help="Apaga todos os alertas ativos e o histórico bruto."):
                            ...
                            # Re-força a tipagem após a limpeza
                            st.session_state.historico_alertas = st.session_state.historico_alertas.astype({
                                'Rodada': 'object', 'RA_Soma': 'object', 'R': 'object', 'C': 'object', 
                                'Resultado_Vela': 'object', 'Rodada_Resultante': 'object'
                            })
...
# INPUT SIMPLIFICADO: usa apenas 2 linhas (Horário Focado, Resultado Vela)
dados_resultado = st.text_area(
    "Cole o **Horário Focado** e o **Resultado da Vela** (em linhas separadas).",
    height=120,
    placeholder="Exemplo:\n11:10:23\n15.55x"
)
...
if st.button("Atualizar Resultado", type="secondary"):
    ...
    # O Horário Focado é a primeira linha
    horario_alvo_res = linhas_res[0] if len(linhas_res) >= 1 else None
    
    # O Resultado da Vela é a última linha
    ...
    if horario_alvo_res and resultado_vela_input:
        ...
        if not re.match(r'\d{2}:\d{2}:\d{2}', horario_alvo_res):
            ...
            # Garante que a coluna Resultado_Vela existe no DataFrame de sessão (caso o histórico tenha sido limpo)
            if 'Resultado_Vela' not in st.session_state.historico_alertas.columns:
                 st.session_state.historico_alertas['Resultado_Vela'] = '-'
...
if not st.session_state.historico_alertas.empty:
    ...
    df_exibicao['ID de Origem'] = df_exibicao['ID de Origem'] # Contém a Rodada Resultante
    df_exibicao['Det. RA/C'] = df_exibicao['Sub_Origem_C'].apply(lambda x: x if x else '-')
    
    # Colunas a serem mantidas e ordenadas na exibição final
