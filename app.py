...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    # NOTA: Não precisamos dos digitos do horário para o cálculo de alertas, mas mantemos para o R
    ...
    # Faltantes de H ainda são calculados, mas NÃO serão usados para gerar alertas.
    ...
    def adicionar_horario(soma: int, origem_completa: str):
        ...
        # Lógica para extrair o nome simplificado (V, H, VT, HT, HM)
    ...
    # ----------------------------------------------------
    # --- NOVO: SOMENTE ALERTAS BASEADOS NA VELA (V) ---
    # ----------------------------------------------------
    ...
    # ----------------------------------------------------
    # --- REMOVIDO: TODOS OS ALERTAS BASEADOS NO HORÁRIO (H) ---
    # ----------------------------------------------------
    # O código que adicionava H x 1, H x 10, HT, e HM foi removido.
    ...
    while i < len(horarios_brutos_list):
        ...
        horarios_consolidados.append({
            ...
            'Origem': ' / '.join(origens_consolidadas), # Ex: 'V / VT'
            'R': r_formatado,
