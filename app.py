...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    for col in COLUNAS_ESTADO:
        if col not in novo_df.columns: 
            if col in ['R', 'C', 'RA_Soma', 'Resultado_Vela']:
                ...
                novo_df[col] = ''
            elif col in ['Timestamp_dt']:
                novo_df[col] = None 
            elif col in ['Rodada_Resultante']:
                 novo_df[col] = None # Mantém None para tratamento de string
    ...
    # Garante que 'Rodada_Resultante' seja tratado como string ANTES de qualquer concatenação.
    for col in ['Rodada_Resultante', 'RA_Soma', 'R', 'C', 'Resultado_Vela']:
         # Se for None/NaN, preenche com '-' e converte para string.
         novo_df_limpo[col] = novo_df_limpo[col].fillna('-').astype(str).replace('None', '-')
         historico_atualizado[col] = historico_atualizado[col].fillna('-').astype(str).replace('None', '-') # Garante o mesmo tratamento no histórico
    ...
    for _, row in historico_completo.iterrows():
        ...
        # 'Rodada_Resultante' já deve ser string ou '-' neste ponto
    ...
    for horario, data in Consolidado_dict.items():
        ...
        if rodadas_resultantes_validas:
            # Ordena as rodadas resultantes numericamente
            id_origem_final = ', '.join(sorted(list(set(rodadas_resultantes_validas)), key=int))
        else:
            # Caso não haja rodada resultante válida, usa a Rodada Original para ID de Origem
        ...
        dados_finais.append({
            ...
            'Rodada_Resultante': rodadas_resultantes_validas[0] if rodadas_resultantes_validas else None 
        })
    ...
    if len(linhas) < 3:
        ...
        try:
            ...
            with st.spinner(f'Analisando Rodada {rodada_int} e consolidando histórico...'): 
                ...
                # INICIALIZAÇÃO CORRIGIDA PARA EVITAR NAMEERROR
                r_final = 'N/A'
                cotacao_final = 'N/A'
