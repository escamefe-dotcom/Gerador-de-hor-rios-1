    ...
    if vela_float_total < 10.00:
        ...
        try:
            ...
            if soma_digitos_fracao == 0:
                return None, None, None, None, None, None, '0' # Retorna 0 para a Soma RA
...
# NOVO CAMPO DE ESTADO: Resultado_Vela
if 'historico_alertas' not in st.session_state:
    st.session_state.historico_alertas = pd.DataFrame(columns=[
        ...
        'Sub_Origem_C', 'Timestamp_dt', 'Resultado_Vela' # <- NOVO CAMPO
    ])
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    # Adiciona a coluna Resultado_Vela se estiver faltando (para compatibilidade com estado anterior)
    if 'Resultado_Vela' not in historico_atualizado.columns:
         historico_atualizado['Resultado_Vela'] = '-'
    ...
    # Garante que o novo_df tenha a coluna Resultado_Vela
    if 'Resultado_Vela' not in novo_df.columns:
        novo_df['Resultado_Vela'] = '-'
    ...
    for _, row in historico_completo.iterrows():
        ...
        if chave_consolidada not in consolidado_dict:
            consolidado_dict[chave_consolidada] = {
                ...
                'Resultado_Vela': row['Resultado_Vela'] # NOVO
            }
        else:
            ...
            # Garante que o Resultado_Vela se mantenha se já foi preenchido
            if consolidated_dict[chave_consolidada]['Resultado_Vela'] == '-' and row['Resultado_Vela'] != '-':
    ...
    for horario, data in consolidado_dict.items():
        ...
        # Consolida RA_Soma e Rodada para manter no histórico
        ...
        dados_finais.append({
            ...
            'Resultado_Vela': data['Resultado_Vela'] # NOVO
        })
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    with st.sidebar:
                        ...
                        if st.button("Limpar Histórico Completo", help="Apaga todos os alertas ativos e o histórico bruto."):
                            st.session_state.historico_alertas = pd.DataFrame(columns=[
                                'Rodada', 'RA_Soma', 'Horário Focado', 'Origem', 'R', 'C', 'Sinalizacao', 
                                'Sub_Origem_C', 'Timestamp_dt', 'Resultado_Vela'
                            ])
                            st.rerun()
...
# --------------------------------------------------------------------------------------
# --- 5. INTERFACE DE ATUALIZAÇÃO MANUAL DE RESULTADO ---
# --------------------------------------------------------------------------------------

st.markdown("---")
st.subheader("✍️ Adicionar Resultado da Vela")

col_res_1, col_res_2, col_res_3 = st.columns([3, 2, 1])

with col_res_1:
    horario_alvo_res = st.text_input("Horário Focado (Ex: 11:10:23)", key="input_horario_res")

with col_res_2:
    resultado_vela_input = st.text_input("Resultado (Ex: 15.55x)", key="input_resultado_res")
    
with col_res_3:
    st.markdown("")
    if st.button("Atualizar Resultado", type="secondary", use_container_width=True):
        if horario_alvo_res and resultado_vela_input:
            
            # Formata o resultado
            resultado_limpo = resultado_vela_input.strip().replace('x', '').replace(',', '.') + 'x'
            
            # Checa se o horário existe no histórico antes de atualizar
            if horario_alvo_res in st.session_state.historico_alertas['Horário Focado'].values:
                ...
                st.success(f"Resultado {resultado_limpo} adicionado em {horario_alvo_res}.")
            else:
                st.error(f"Horário '{horario_alvo_res}' não encontrado no histórico ativo.")
        else:
            st.warning("Preencha o Horário Focado e o Resultado da Vela.")
...
if not st.session_state.historico_alertas.empty:
    ...
    # 1. Cria a coluna principal de Horário
    df_exibicao['Horário'] = df_exibicao['Sinalização'] + ' ' + df_exibicao['Horário Focado']
    
    # 2. Usa a nova coluna Resultado_Vela para a exibição (seu pedido)
    df_exibicao['Resultado Vela'] = df_exibicao['Resultado_Vela']
    
    # 3. Cria a coluna de Detalhes
    df_exibicao['Det. RA/C'] = df_exibicao['Sub_Origem_C'].apply(lambda x: x if x else '-')
    
    # 4. Exclui as colunas internas/redundantes antes de mostrar
    df_exibicao = df_exibicao.drop(
        columns=['Timestamp_dt', 'Sinalização', 'Horário Focado', 'Sub_Origem_C', 'Rodada', 'RA_Soma', 'Resultado_Vela'] 
    ) 
    
    st.dataframe(
        df_exibicao, 
        hide_index=True,
        column_order=['Horário', 'Resultado Vela', 'R', 'C', 'Origem', 'Det. RA/C'], 
        column_config={
