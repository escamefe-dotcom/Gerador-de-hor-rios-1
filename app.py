...
def consolidar_alertas_internamente(horarios_brutos_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    while i < len(horarios_brutos_list):
        ...
        # Consolida Rodada Resultante: prioriza qualquer valor não nulo no grupo (Rodada Alvo)
        # Rodada Resultante é o ID de Origem que o usuário quer.
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    try:
        ...
        # st.error(f"Erro de Validação/Parsing: {e}") # Não exibir erro aqui para não parar o processamento
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    # Filtra alertas antigos e remove colunas de trabalho para re-criação
