...
def gerar_alertas_confluencia(rodada_completa: int, vela_str: str, horario_base_dt: datetime) -> List[Dict[str, Any]]:
    ...
    for item in confluencias:
        ...
        # Calcula a diferença em minutos para o cálculo da Rodada Resultante (Rodada Original + diff_minutos)
        diff_minutos = (horario_alvo_dt.minute - horario_base_dt.minute) % 60
        if horario_alvo_dt.hour > horario_base_dt.hour or (horario_alvo_dt.hour == horario_base_dt.hour and horario_alvo_dt.minute < horario_base_dt.minute):
            diff_minutos += 60 * ((horario_alvo_dt.hour - horario_base_dt.hour) % 24)
        
        if horario_alvo_dt < horario_processamento_dt:
            horario_alvo_dt += timedelta(hours=1)
            # Reajusta a diferença de minutos se o horário avançou de hora (lógica simplificada para fins de rastreio de rodada)
            diff_minutos += 60 # Se avançou 1h, são 60 minutos a mais
        ...
        alertas_confluencia.append({
            ...
            'Minutos_Deslocamento': diff_minutos
        })
...
def consolidar_alertas_internamente(horarios_brutos_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    while i < len(horarios_brutos_list):
        ...
        horarios_consolidados.append({
            ...
            'Rodada_Resultante': rodada_resultante, # Novo Campo Consolidado
        })
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    # 2. Lógica COMPLETA para VELAS >= 10.00x (R, C, V, VT, Confluência)
        ...
        for alerta_c in alertas_confluencia:
            ...
            # Calcula Rodada Resultante para Confluência, se houver deslocamento
            deslocamento_minutos = alerta_c.get('Minutos_Deslocamento', 0)
            rodada_resultante_c = str(rodada_int + deslocamento_minutos)
            
            if horario_alvo_c_minuto in horarios_faltantes_alvo_set:
                # Se houver confluência com V/VT, usamos a rodada resultante de V/VT
                rodada_resultante_c = None 
            else:
                 # Caso C seja isolada, usamos a Rodada Resultante calculada com base no deslocamento de minutos
                 rodada_resultante_c = str(rodada_int + deslocamento_minutos)
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    if 'Rodada_Resultante' not in novo_df.columns:
         novo_df['Rodada_Resultante'] = None
    ...
    for _, row in historico_completo.iterrows():
        ...
        if chave_consolidada not in Consolidado_dict:
            Consolidado_dict[chave_consolidada] = {
                'Rodadas': {row['Rodada']}, # Rodada Original
    ...
    for horario, data in Consolidado_dict.items():
        ...
        # ID DE ORIGEM: Rodada Resultante (Rodada Original + Minutos de Deslocamento)
        ...
        if rodadas_resultantes_validas:
            ...
            # Caso não haja deslocamento (ex: Confluência sem alteração de minuto), mostra a Rodada Original
...
if not st.session_state.historico_alertas.empty:
    ...
    df_exibicao = df_exibicao.drop(
        columns=['Timestamp_dt', 'Sinalização', 'Horário Focado', 'Sub_Origem_C', 'Rodada'] 
    ) 
