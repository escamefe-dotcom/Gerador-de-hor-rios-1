...
def obter_analise_finais_estatica() -> Tuple[pd.DataFrame, Dict[int, int], int]:
    ...
    # DADOS ESTÁTICOS MAIS RECENTES: Top 1 Isolado: 17; Finais: T1(3), T2(0), T3(2), T4(6), T5(7)
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    ...
    for v in faltantes_V:
        ...
        # Deixei o x10 comentado para simplificar o número de alertas por Rodada
    ...
    if vela_float_total < 10.00:
        try:
            # 1. Extrai e soma os dígitos depois da vírgula
                ...
                for i, offset in enumerate(minuto_offsets):
                    ...
                    if offset > 0:
                        ...
                        horarios_brutos_list.append({
                            ...
                            'R': r_formatado,
                            'C': cotacao_C,
                            'Sub_Origem_C': f"RA({sub_origem_ra})" # Usamos Sub_Origem_C para detalhar
                        })
        except Exception as e:
            print(f"Erro no cálculo RA (Rodadas Adicionadas): {e}")
    ...
    for alerta_c in alertas_confluencia:
        ...
        # REGRA DE FILTRO: O sinal C só entra se houver V ou VT (já gerados) no mesmo minuto
    ...
    # 3. CONSOLIDAÇÃO INTERNA DE ALERTAS (Agrupamento V, VT, C, RA no mesmo minuto/segundo)
    ...
    while i < len(horarios_brutos_list):
        ...
        # Lógica de Horário Final (meio do grupo ou último)
        if len(grupo) % 2 == 1:
        ...
        # Consolida Origens Brutas (V, VT, C, RA)
        origens_consolidadas = sorted(list(set(item['Origem_Bruta'] for item in grupo)))
        
        # Consolida Sub_Origens (detalhes do C e do RA)
        ...
        horarios_consolidados.append({
            ...
            'Origem': ' / '.join(origens_consolidadas), # Agora pode ter RA
            'R': r_consolidado,
            'C': c_consolidado, 
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    # Limpa alertas passados (mantendo apenas alertas futuros ou do horário base)
    ...
    for _, row in historico_completo.iterrows():
        ...
        if chave_consolidada not in consolidado_dict:
            ...
            consolidado_dict[chave_consolidada] = {
                'Rodadas': {row['Rodada']},
                'Origens': set(row['Origem'].split(' / ')), # Armazena {'V', 'VT', 'C', 'RA'}
                'R': row['R'],
                'C': row['C'],
    ...
    for horario, data in consolidado_dict.items():
        ...
        # Converte o SET de Origens de volta para STRING ordenada (C / RA / V / VT)
