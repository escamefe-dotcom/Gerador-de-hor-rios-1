...
def obter_analise_finais_estatica() -> Tuple[pd.DataFrame, Dict[int, int], int]:
    ...
    # Criando o DataFrame para exibição na sidebar (CORRIGIDO)
    dados_exibicao = [
        # 1. O Minuto 50: Prioridade Máxima (🎯)
        {'Sinal': "🎯", 'Minuto Exemplo': f"{top_1_minuto_completo:02d}", 'Final': f"{0} (Minuto Isolado)"},
        # 2. Demais Top Finais (Exibindo a classificação na coluna 'Final')
...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    """
    Consolida o histórico de alertas, aplicando a sinalização (T1 a T5) baseada 
    no ranking dos Finais de Minuto.
    """
    ...
    for _, row in historico_completo.iterrows():
        ...
        final_minuto = minuto_focado % 10 # Extrai o final do minuto
        ...
        # 2. Demais do TOP 5: Mapeamento pelo RANK do Final de Minuto
            ...
            # Usamos o rank (1 a 5) para montar o sinal (T2), (T3), etc.
            # Se for o rank 1, ele deveria ser T1, mas T1 já é usado pelo minuto isolado.
            # Portanto, mapeamos T1 do rank para T2, T2 para T3, etc.
            
            # Novo Mapeamento: T1 isolado é (T1). Os ranks dos finais são T1 a T5.
            # O final rank 1 (8) terá a sinalização (T1). O final rank 2 (7) terá (T2), e assim por diante.
            sinalizacao = f"(T{rank})"
            
            # Se o minuto isolado TOP 1 (50) pertencer a um rank, ele já foi pego acima.
            # Mas se, por acaso, o rank do final do minuto 50 (que é 3) aparecer aqui, 
            # ele não deve sobrescrever o (T1) se já estiver lá.
            if minuto_focado == top_1_minuto_00_59:
                 sinalizacao = "(T1)"
        ...
        if chave_consolidada not in consolidado_dict:
            ...
            # Lógica de Prioridade: T1 (isolado) > T1 (rank 1) > T2 > T3 > T4 > T5
            
            # Obtém a prioridade numérica (quanto menor, mais prioritário)
            def obter_prioridade(sinal):
                if sinal == "(T1)":
                    return 0 # Mais prioritário (isolado)
                match = re.search(r'\(T(\d)\)', sinal)
                if match:
                    return int(match.group(1))
                return 99 # Menos prioritário
        ...
        try:
            ...
            with st.spinner(f'Analisando Rodada {rodada} e consolidando histórico...'): 
                ...
                if novo_df_bruto is not None and not novo_df_bruto.empty:
                    ...
                    # 4. Consolida Alertas e aplica Sinalização (USANDO NOVO DICIONÁRIO DE RANKING)
                    ...
                    with st.sidebar:
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "Final": st.column_config.TextColumn("Classificação") # CORRIGIDO
                            }
                        )
    ...
    st.dataframe(
        ...
        column_config={
            "Horário": st.column_config.TextColumn(
                ...
                help="(T1): Minuto Top 1 Isolado (50) ou Final T1(8) | (T2) Final T2(7), etc." 
            )
        }
    )
