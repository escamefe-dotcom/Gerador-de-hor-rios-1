...
def consolidar_historico(novo_df, horario_base_dt, rank_finais: Dict[int, int], top_1_minuto_00_59: int):
    ...
    for horario, data in Consolidado_dict.items():
        ...
        # O ID de Origem será a RA_Soma (se houver) ou a Rodada (se não houver RA)
...
# --------------------------------------------------------------------------------------
# --- 5. INTERFACE DE ATUALIZAÇÃO MANUAL DE RESULTADO (ORGAIZADA POR HORÁRIO E RESULTADO) ---
# --------------------------------------------------------------------------------------

st.markdown("---")
st.subheader("✍️ Adicionar Resultado da Vela")

# Mantemos a entrada de texto para copiar e colar mais rápido, mas ajustamos a instrução.
...
if st.button("Atualizar Resultado", type="secondary"):
    ...
    # Espera-se 2 linhas: Horário Focado e Resultado.
    if len(linhas_res) >= 2:
        horario_alvo_res = linhas_res[0]
        resultado_vela_input = linhas_res[1]
    else:
        horario_alvo_res = None
        resultado_vela_input = None
...
if not st.session_state.historico_alertas.empty:
    ...
    # 3. ID de Origem (Rodada ou RA Soma)
    df_exibicao['ID de Origem'] = df_exibicao.apply(
