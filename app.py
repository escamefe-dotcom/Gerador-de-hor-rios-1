...
def analisar_top_minutos_completos(df_bruto: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if df_bruto.empty:
        return pd.DataFrame(columns=['Minuto', 'Contagem', 'Sinalização'])
    ...
    df_top_5 = contagem_minutos.head(5).copy() # Usar copy para evitar SettingWithCopyWarning
    
    # Aplica sinalização: 🚀 para o Top 1, 🔥 para o Top 2 ao 5
...
def analisar_e_gerar_alertas(rodada, vela_str, horario_str):
    # ... (Corpo da função de Geração de Alertas - Inalterado) ...
    try:
